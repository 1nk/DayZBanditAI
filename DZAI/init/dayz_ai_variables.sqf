...
DZAI_spawnRandom = 15;				//Number of randomly-placed AI to spawn on server start. These AI are randomly placed on the map in the same way as Heli Crashes.
DZAI_spawnRandomDelay = 30;			//Time to wait (seconds) before respawning a randomly-placed AI, and time in between each AI spawn at server start.
...
DZAI_weaponGrades = [0,1,2,3];		//All possible weapon grades. Generally, 0: Residential, 1: Military, 2: MilitarySpecial, 3: Heli Crash. (MUST change DZAI_weaponGrades and DZAI_gradeChances together to avoid issues!)
DZAI_gradeChances = [0.17,0.56,0.20,0.07]; //Probabilities for generating weapon grades.
...
//To make changes for a specific DayZ map, edit the appropriate config file in configs folder. 
//Rule of thumb: If it doesn't belong in the official DayZ mod, it doesn't belong here. Edit the map-specific config file instead.
...
DZAI_DefaultMedicals1 = ["ItemBandage", "ItemBandage", "ItemPainkiller"];
