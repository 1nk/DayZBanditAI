	...
	*/
	...
	if (_totalAI > 0) then {						// Only run script if there is at least one bandit to spawn
		for "_i" from 1 to _totalAI do {
			//_p = _bldgpos call BIS_fnc_selectRandom;
			_pos = [_p, 0, 30, 0, 0, 20, 0] call BIS_fnc_findSafePos;
			_types = ["CamoWinterW_DZN", "CamoWinter_DZN"];	// AI Bandit types
			...
			null = [killSquadGrp,_pos,_patrold] execVM "BIN_taskPatrol.sqf";
			//null = [killSquadGrp,_pos,_patrold] execVM "BIN_taskDefend.sqf";
